...
class BackgroundNotification(private val context: Context) {
    ...
    fun notify(metadata: MediaPlayback.MediaMetaData) {
        ...
        val playPause = PendingIntent.getBroadcast(context, 1, LocalIntent.createMediaKeyEvent(playPauseKey), PendingIntent.FLAG_UPDATE_CURRENT)
        val next = PendingIntent.getBroadcast(context, 1, LocalIntent.createMediaKeyEvent(nextKey), PendingIntent.FLAG_UPDATE_CURRENT)
        val prev = PendingIntent.getBroadcast(context, 1, LocalIntent.createMediaKeyEvent(prevKey), PendingIntent.FLAG_UPDATE_CURRENT)
