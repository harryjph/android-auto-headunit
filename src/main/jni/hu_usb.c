...
#include "libs/headers/libusb/libusb.h"
...
//#endif
    ...
    while (continue_transfer) {
        ...
        if (ms_duration > 400)
            loge ("ms_duration: %d dir: %s  len: %d  bytes_xfrd: %d  total_bytes_xfrd: %d  usb_err: %d (%s)  errno: %d (%s)",
                  ms_duration, dir, len, bytes_xfrd, total_bytes_xfrd, usb_err,
                  iusb_error_get(usb_err), errno, strerror(errno));
...
int iusb_control_transfer(libusb_device_handle *usb_hndl, uint8_t req_type, uint8_t req_val,
                          uint16_t val, uint16_t idx, byte *buf, uint16_t len, unsigned int tmo) {

    //if (ena_log_verbo)
    logd ("Start usb_hndl: %p  req_type: %d  req_val: %d  val: %d  idx: %d  buf: %p  len: %d  tmo: %d",
    ...
    if (usb_err < 0) {
        loge ("Error usb_err: %d (%s)  usb_hndl: %p  req_type: %d  req_val: %d  val: %d  idx: %d  buf: %p  len: %d  tmo: %d",
              usb_err, iusb_error_get(usb_err), usb_hndl, req_type, req_val, val, idx, buf, len,
              tmo);
        return (-1);
    }
    //if (ena_log_verbo)
    logd ("Done usb_err: %d  usb_hndl: %p  req_type: %d  req_val: %d  val: %d  idx: %d  buf: %p  len: %d  tmo: %d",
...
int iusb_oap_start() {
    ...
    ms_sleep(
            50);//1);                                                        // Sometimes hangs on the next transfer
...
//  {logd ("Done USB_VID_GOO 1"); return (0); }
    ...
    iusb_control_transfer(iusb_dev_hndl, req_type, req_val, val, ACC_IDX_MAN, AAP_VAL_MAN,
                          strlen(AAP_VAL_MAN) + 1, tmo);
    iusb_control_transfer(iusb_dev_hndl, req_type, req_val, val, ACC_IDX_MOD, AAP_VAL_MOD,
                          strlen(AAP_VAL_MOD) + 1, tmo);
...
int iusb_deinit() {                                              // !!!! Need to better reset and wait a while to kill transfers in progress and auto-restart properly
    ...
    int usb_err = libusb_release_interface(iusb_dev_hndl,
...
int iusb_init(byte ep_in_addr, byte ep_out_addr) {
    ...
    if (iusb_best_vendor == 0 ||
        iusb_best_device == NULL) {                                             // If no vendor...
        loge ("Error device not found iusb_best_vendor: 0x%04x  iusb_best_device: %p",
              iusb_best_vendor, iusb_best_device);
        libusb_free_device_list(list,
                                1);                                // Free device list now that we are finished with it
        return (-1);
    }
    logd ("Device found iusb_best_vendor: 0x%04x  iusb_best_device: 0x%04x  iusb_best_man: \"%s\"  iusb_best_pro: \"%s\"",
    ...
    if ((ep_in_addr == 255 && ep_out_addr == 0) ||
        file_get("/sdcard/hu_disable_selinux_chmod_bus")) {    //
        // Disable SELinux and open /dev/bus permissions for SUsb
        int ret = system(
                "su -c \"setenforce 0 ; chmod -R 777 /dev/bus 1>/dev/null 2>/dev/null\""); // !! Binaries like ssd that write to stdout cause C system() to crash !
        logd ("iusb_usb_init system() w/ su ret: %d", ret);

        ret = system(
                "chmod -R 777 /dev/bus 1>/dev/null 2>/dev/null"); // !! Binaries like ssd that write to stdout cause C system() to crash !
...
//*/
    ...
    if (usb_err != 0) {
        logd ("Expected Error libusb_get_config_descriptor usb_err: %d (%s)  errno: %d (%s)",
              usb_err, iusb_error_get(usb_err), errno,
              strerror(errno));    // !! ???? Normal error now ???
        //return (-1);
    ...
    for (idx = 0; idx < num_int; idx++) {                              // For all interfaces...
        ...
        int j = 0;
        for (j = 0;
             j < inter->num_altsetting; j++) {                    // For all alternate settings...
            ...
            for (k = 0; k < num_eps; k++) {                                // For all endpoints...
                ...
                if (epdesc->bDescriptorType == LIBUSB_DT_ENDPOINT) {          // 5
                    if ((epdesc->bmAttributes & LIBUSB_TRANSFER_TYPE_MASK) ==
                        ...
                        if (iusb_ep_in > 0 && iusb_ep_out >
                                              0) {                    // If we have both endpoints now...
                            libusb_free_config_descriptor(config);

                            if (ep_in_addr != 255 &&
                                (iusb_ep_in != ep_in_addr || iusb_ep_out != ep_out_addr)) {
                                logw ("MISMATCH Done endpoint search iusb_ep_in: 0x%02x  iusb_ep_out: 0x%02x  ep_in_addr: 0x%02x  ep_out_addr: 0x%02x",
                                      iusb_ep_in, iusb_ep_out, ep_in_addr,
                                      ep_out_addr);    // Favor libusb over passed in
                            }
                            else
                                logd ("Match Done endpoint search iusb_ep_in: 0x%02x  iusb_ep_out: 0x%02x  ep_in_addr: 0x%02x  ep_out_addr: 0x%02x",
                                      iusb_ep_in, iusb_ep_out, ep_in_addr, ep_out_addr);
    ...
    loge ("Error in and/or out endpoints unknown iusb_ep_in: 0x%02x  iusb_ep_out: 0x%02x",
          iusb_ep_in, iusb_ep_out);
    libusb_free_config_descriptor(config);
