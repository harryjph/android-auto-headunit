...
public class WifiManager {
    ...
    private class WifiStartTask extends AsyncTask<Object, Void, Boolean> {
        ...
        protected Boolean doInBackground(Object... params) {//(Void... params) {// (Params... p) {//Void... v) {//Void... params) {
            AppLog.i("wifi_long_start start ");
        ...
        protected void onPostExecute(Boolean result) {//String result) {
            AppLog.i("wifi_long_start done: " + result);
            onWifiStarted();
        }
    }

    void deinitP2P() {
        AppLog.i("m_wifidir_mgr: " + m_wifidir_mgr + "  m_wifidir_chan: " + m_wifidir_chan + "  m_wifidir_bcr: " + m_wifidir_bcr);
...
//*
        m_wifidir_mgr.stopPeerDiscovery(m_wifidir_chan, new WifiP2pManager.ActionListener() {
            ...
            public void onSuccess() {
                AppLog.i("stopPeerDiscovery Success");
            }

            @Override
            public void onFailure(int reasonCode) {
                AppLog.e("stopPeerDiscovery Failure reasonCode: " + reasonCode);
            }
        });
//*/
        WifiP2pManager.ActionListener wal = new WifiP2pManager.ActionListener() {
            @Override
            public void onSuccess() {
                AppLog.i("stopPeerDiscovery/cancelConnect/removeGroup Success");
            }

            @Override
            public void onFailure(int reasonCode) {
                AppLog.e("stopPeerDiscovery/cancelConnect/removeGroup Failure reasonCode: " + reasonCode);
            }
        };
    ...
    void initP2P() {
        ...
        WifiP2pManager.ActionListener wal = new WifiP2pManager.ActionListener() {
            @Override
            public void onSuccess() {
                AppLog.i("createGroup Success");                            // This is only triggered after we have a connection
            }

            @Override
            public void onFailure(int reasonCode) {
                AppLog.e("createGroup Failure reasonCode: " + reasonCode);  // Already exists ?: reateGroup Failure reasonCode: 2
            }
        };
    ...
    WifiP2pManager.PeerListListener myPeerListListener = new WifiP2pManager.PeerListListener() {
        ...
        public void onPeersAvailable(WifiP2pDeviceList peers) {
            AppLog.i("myPeerListListener onPeersAvailable peers: " + peers);
        }
    };
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(WifiP2pManager.WIFI_P2P_STATE_CHANGED_ACTION)) {          // Check to see if Wi-Fi is enabled and notify appropriate activity
                ...
                if (state == WifiP2pManager.WIFI_P2P_STATE_ENABLED) {
                    AppLog.i("STATE_CHANGED Wifi P2P is enabled");
                } else {
                    AppLog.i("STATE_CHANGED Wi-Fi P2P is not enabled");
                }
            } else if (action.equals(WifiP2pManager.WIFI_P2P_PEERS_CHANGED_ACTION)) {
                AppLog.i("PEERS_CHANGED");        // Request currently available peers from the wifi p2p manager. This is an asynchronous call and the calling activity is notified with a callback on PeerListListener.onPeersAvailable()
                if (m_wifidir_mgr != null) {
                    m_wifidir_mgr.requestPeers(m_wifidir_chan, myPeerListListener);
                }
            } else if (action.equals(WifiP2pManager.WIFI_P2P_CONNECTION_CHANGED_ACTION)) {
                AppLog.i("CONNECTION_CHANGED");          // Respond to new connection or disconnections
            } else if (action.equals(WifiP2pManager.WIFI_P2P_THIS_DEVICE_CHANGED_ACTION)) {
                AppLog.i("THIS_DEVICE_CHANGED");          // Respond to this device's wifi state changing
            } else if (action.equals(UiModeManager.ACTION_ENTER_CAR_MODE)) {
                AppLog.i("ACTION_ENTER_CAR_MODE");
            } else if (action.equals(UiModeManager.ACTION_EXIT_CAR_MODE)) {
                AppLog.i("ACTION_EXIT_CAR_MODE");
            } else if (action.equals(UiModeManager.ACTION_ENTER_DESK_MODE)) {
                AppLog.i("ACTION_ENTER_DESK_MODE");
            } else if (action.equals(UiModeManager.ACTION_EXIT_DESK_MODE)) {
                AppLog.i("ACTION_EXIT_DESK_MODE");
            } else {
                AppLog.e("OTHER !! ??");
            }
        }
