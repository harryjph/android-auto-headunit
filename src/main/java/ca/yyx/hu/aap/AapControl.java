...
class AapControl {
    ...
    private int ping_request(int chan, byte[] buf, int len) {                  // Ping Request
        if (len != 4 || buf[2] != 0x08)
    ...
    private int navigation_focus_request(int chan, byte[] buf, int len) {                  // Navigation Focus Request
        if (len != 4 || buf[2] != 0x08)
