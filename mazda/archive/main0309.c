...
void *send_aa_cmd_thread(void *arguments)
{
	...
	int chan = args->cmd_buf[0];
	int res_len = 0;
	int ret = 0;

	pthread_mutex_lock (&mutexsend);

	ret = hu_aap_enc_send (args->retry, chan, (args->cmd_buf)+4, args->cmd_len - 4);
...
static int gst_pipeline_init(gst_app_t *app)
{
	...
	app->pipeline = (GstPipeline*)gst_parse_launch("appsrc name=mysrc is-live=true do-timestamp=true format=3 min-latency=0 max-latency=10000000 ! video/x-h264, width=(int)800, height=(int)480 ! vpudec low-latency=true framedrop=true frame-plus=2 framedrop-level-mask=0x200 ! mfw_v4lsink async=false", &error);
...
//	g_signal_connect(app->src, "need-data", G_CALLBACK(start_feed), app);
	...
	g_signal_connect(app->src, "need-data", G_CALLBACK(read_data), app);
