...
char *iaad_msg_type_str_get(int msg_type, unsigned char *src,
                            int len) {   // Source:  HU = HU > AA   AA = AA > HU
...
unsigned int iaad_vint_dec(int log_dmp, unsigned char *buf, unsigned int len,
                           unsigned long *vintp) {  // Varint decode
...
unsigned int iaad_dmp_arry(int log_dmp, unsigned int n, unsigned char *buf, unsigned int len) {
    ...
    adj = iaad_dmp_n(0, n + 1, buf,
                     alen);                             // Try first without error logging to see if this is a valid array
    if (adj != alen || adj >
                       8192) {                                    //  "|| rmv + adj != len) {" only works if array is last item in packet
...
unsigned int iaad_dmp_n(int log_dmp, unsigned int n, unsigned char *buf, unsigned int len) {
    ...
    if (lft != 0 || rmv != len || rmv < 0) if (log_dmp)
