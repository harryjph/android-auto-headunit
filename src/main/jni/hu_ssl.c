...
#include "libs/headers/openssl/err.h"
...
int hu_ssl_handshake() {
    ...
    SSL_set_bio(hu_ssl_ssl, hu_ssl_rm_bio,
                hu_ssl_wm_bio);                                  // Read from memory, write to memory

    BIO_set_write_buf_size (hu_ssl_rm_bio,
    ...
    while (!hs_finished && hs_ctr++ < 2) {
        ...
        ret = BIO_read(hu_ssl_wm_bio, &hs_buf[6], sizeof(hs_buf) -
                                                  6);         // Read from the BIO Client request: Hello/Key Exchange
        if (ret <= 0) {
        ...
        int len = ret + 6;
        ret = hu_aap_tra_set(AA_CH_CTR, 3, 3, hs_buf,
                             len);              // chan:0/AA_CH_CTR   flags:first+last    msg_type:SSL
        ret = hu_aap_tra_send(hs_buf, len,
                              2000);                       // Send Client request to AA Server
        if (ret <= 0 || ret != len) {
        ...
        ret = BIO_write(hu_ssl_rm_bio, &hs_buf[6],
                        ret - 6);          // Write to the BIO Server response
        if (ret <= 0) {
