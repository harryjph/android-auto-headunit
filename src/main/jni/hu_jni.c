...
*/
        ...
        if (cmd_len >= 8 && cmd_buf[5] == 15) {      // If byebye...
            logd ("Byebye");
            ms_sleep(100);
            ret = hu_aap_stop();
        }
        ...
        if (vid_bufs > 0 ||
            aud_bufs > 0)                                 // If any queue audio or video...
    ...
    if (vid_bufs <= 0 &&
        aud_bufs <= 0) {                               // If no queued audio or video...
        ret = hu_aap_mic_get();
        if (ret >=
            1) {// && ret <= 2) {                                  // If microphone start (2) or stop (1)...
    ...
    if (dq_buf ==
        NULL)                                                 // If no audio... (Audio has priority over video)
...
JNIEXPORT jint Java_ca_yyx_hu_aap_AapTransport_native_1aa_1cmd(JNIEnv *env, jobject thiz,
