...
/*
? External APIs used from HeadUnitActivity:
...
//Wifi messes this up
//        if (! m_stopping)
//          Utils.ms_sleep (1);//3);                                   // Up to 330 frames per second
...
//Utils.loge ("5");
    ...
    public void touch_send(byte action, int x, int y) {                  // Touch event send. Called only by HeadUnitActivity:touch_send()
        ...
        while (new_touch) {                                                 // While previous touch not yet processed...
            ...
            //Utils.loge ("BEFORE Utils.ms_sleep (50);");
            Utils.ms_sleep(50);                                             // Wait a bit
            //Utils.loge ("AFTER  Utils.ms_sleep (50);");
        }

        //synchronized (this) {
  /*  if (action == 2) {
        long this_move_ms = Utils.tmr_ms_get ();
        if (last_move_ms + 50 > this_move_ms) {
          Utils.loge ("Dropping motion this_move_ms: " + this_move_ms + "  last_move_ms: " + last_move_ms);
          return;
        }
...
//
    ...
    public int transport_start(Intent intent) {                          // USB Transport Start. Called only by HeadUnitActivity:onCreate()
        ...
        if (action != null &&
            ...
            Utils.logd("device_list.size() ret: " + ret);
...
//          }
    ...
    private void usb_detach_handler(UsbDevice device) {                  // Handle detached device.  Called only by usb_receiver() if device detached while app is running (only ?)
        ...
        String usb_dev_name = usb_dev_name_get(device);
        Utils.logd("usb_detach_handler m_usb_connected: " + m_usb_connected + "  usb_dev_name: " + usb_dev_name);  // device: " + device + "  m_usb_device: " + m_usb_device + "
    ...
    private void usb_disconnect() {
        Utils.logd("usb_disconnect");// m_usb_device: " + m_usb_device);
    ...
    private void usb_connect(UsbDevice device) {                         // Attempt to connect. Called only by usb_attach_handler() & presets_select()
        Utils.logd("usb_connect m_autolaunched: " + m_autolaunched);// device: " + device);
...
*/
        ...
        if (m_autolaunched)                                                 // Not used now
            Utils.logd("Have Implicit USB Permission");
        else
            Utils.logd("Have Explicit USB Permission");
...
/*  !! Doesn't work !!
    ...
    private int usb_open(UsbDevice device) {                             // Open USB device connection & claim interface. Called only by usb_connect()
        ...
        if (m_usb_dev_conn == null) {
            Utils.logw("Could not obtain m_usb_dev_conn for device: " + device);
            return (-1);                                                      // Done error
        }
        Utils.logd("Device m_usb_dev_conn: " + m_usb_dev_conn);
...
// gs3/no2 internal: dev_name: /dev/bus/usb/001/002  dev_class: 2  dev_sclass: 0  dev_dev_id: 1002  dev_proto: 0  dev_vend_id: 5401  dev_prod_id: 32        0x1519 : 0x0020   Comneon : HSIC Device
        ...
        String dev_prod = "";
        String dev_ser = "";
        if (Utils.android_version >= 21) {                                 // Android 5.0+ only
            try {
...
/*  if (dev_man.equals (""))
    ...
    private String[] usb_list_name = new String[HeadUnitActivity.PRESET_LEN_USB];
    private UsbDevice[] usb_list_device = new UsbDevice[HeadUnitActivity.PRESET_LEN_USB];
    ...
    public void presets_select(int idx) {                                // Called only by HeadUnitActivity:preset_select_lstnr:onClick()
        Utils.logd("idx: " + idx + "  name: " + usb_list_name[idx] + " device : " + usb_list_device[idx]);
    ...
    private void usb_add(String name, UsbDevice device) {
        if (usb_list_num >= HeadUnitActivity.PRESET_LEN_USB) {
        ...
        usb_list_num++;
        mHeadUnitActivity.presets_update(usb_list_name);
        usb_log();
    }
...
/* Not used
  private int usb_io (UsbEndpoint ep, byte [] buf, int len, int tmo) {
    if (m_usb_dev_conn == null) {
      Utils.loge ("m_usb_dev_conn: " + m_usb_dev_conn);
      return (-1);
    }
    int ret = m_usb_dev_conn.bulkTransfer (ep, buf, len, tmo);          // This form good for API level 12+, IDX form 4.3+ only
    if (ret < 0) {
      Utils.loge ("Bulk transfer failed ret: " + ret);
    }
    Utils.logd ("Bulk transfer ret: " + ret);
    return (ret);
  }
